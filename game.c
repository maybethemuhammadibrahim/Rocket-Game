...
void game(){
    ...
    while (!gameOver) {
        ...
        // Improved input handling - check for all pressed keys
        while (_kbhit()) {
            char key = _getch();
            switch (key) {
                case 'a':
                ...
                case 'D':
                    ...
                    break;
                case ' ':
                    // Shooting bullet - find first available bullet slot
...
void displayHighScores() {
    ...
    gotoxy(10, 2);
    printf(PURPLE "HIGH SCORES" reset);
    for (int i = 0; i < MAX_HIGHSCORES; i++) {
        gotoxy(10, 4 + i);
...
void updateHighScores(int newScore) {
    ...
    if (insertIndex != -1) {
        ...
        scanf("%19s", highScores[insertIndex].name); // Limit input to prevent buffer overflow
    }
